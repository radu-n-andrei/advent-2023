...
object Digger {
    ...
    def dig(orders: List[TrenchOrder]): List[DirectedCoordinate] = {
        ...
        def keepDigging(current: Coordinate, orders: List[TrenchOrder], acc: List[DirectedCoordinate]): List[DirectedCoordinate] = {
            orders match {
                case Nil => acc
                case o :: os =>
                    val trench = o.direction.move(current, o.distance).map(c => DirectedCoordinate(o.direction, c))
    ...
    def normalizeTrench(trench: List[DirectedCoordinate]): List[DirectedCoordinate] = {
        ...
        if(xF != 0 || yF != 0) 
            trench.map(c => c.copy(coordinate = Coordinate(c.coordinate.x - xF, c.coordinate.y - yF)))
        else trench    
    }
}
